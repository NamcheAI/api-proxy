...
tar czf - --exclude=.git --exclude=.github --exclude=node_modules --exclude=.env . | ssh "admin@$HOST" 'set -e; sudo install -d -o deploy -g deploy '"$DEPLOY_PATH"'; sudo tar xzf - -C '"$DEPLOY_PATH"'; sudo chown -R deploy:deploy '"$DEPLOY_PATH"'; sudo -u deploy sh -lc "cd '"$DEPLOY_PATH"' && npm ci --omit=dev"; sudo systemctl restart api-proxy; sudo systemctl is-active --quiet api-proxy && echo DEPLOY_OK'
